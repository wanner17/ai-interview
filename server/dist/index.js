...
try {
    ...
    console.log('⚠️ GCP STT Mock 모드');
}
const hasOpenAIKey = !!process.env.OPENAI_API_KEY;
const openai = new openai_1.default({ apiKey: process.env.OPENAI_API_KEY || 'dummy_key' });
...
app.post('/api/generate-questions', upload.single('resume'), async (req, res) => {
    try {
        ...
        if (mimetype === 'application/pdf') {
            ...
            res.status(400).json({ error: 'PDF, DOCX, TXT 파일만 지원합니다.' });
            return;
        }
        if (!resumeText.trim()) {
            res.status(400).json({ error: '텍스트 추출 실패' });
            return;
        }
        if (!hasOpenAIKey) {
        ...
        res.json({ questions: parsed.questions.map((q, i) => `${i + 1}. ${q}`) });
    }
    catch (err) {
...
app.post('/api/pt-generate-topic', async (req, res) => {
    try {
        const { category = 'business', customTopic } = req.body;
        if (customTopic?.trim()) {
            ...
            res.json({ topic: customTopic.trim(), outline, timeLimit: 5 });
            return;
        }
        if (!hasOpenAIKey) {
            const def = PT_DEFAULT_TOPICS[category] || PT_DEFAULT_TOPICS.business;
            res.json({ ...def, timeLimit: 5 });
            return;
        }
...
app.post('/api/debate-topic', async (req, res) => {
    try {
        const { category = 'social', customTopic, debateType = 'pro-con' } = req.body;
        if (!hasOpenAIKey) {
            const def = DEBATE_DEFAULTS[category] || DEBATE_DEFAULTS.social;
            res.json(def);
            return;
        }
        const catNames = { social: '사회/노동', tech: 'IT/기술', education: '교육', economy: '경제', environment: '환경/에너지' };
        ...
        console.error('⚠️ [DebateTopic]', err);
        res.status(500).json({ error: '주제 생성 실패' });
    }
});
// ── AI 헬퍼 함수들 ────────────────────────────────────────────────────────────
async function generateFollowupQuestion(question, answer, language) {
...
async function generateFeedback(question, answer, focusScore, language, isFollowup) {
...
io.on('connection', (socket) => {
    ...
    socket.on('start_audio_stream', () => {
        ...
        recognizeStream = speechClient.streamingRecognize({
            config: { encoding: 'WEBM_OPUS', sampleRateHertz: 48000, languageCode: sessionLanguage },
            interimResults: true,
        })
            .on('error', (err) => { console.error(`⚠️ [STT]`, err.message); if (recognizeStream)
            recognizeStream.destroy(); recognizeStream = null; })
            .on('data', (data) => {
            const r = data.results[0];
            if (r?.alternatives[0])
                socket.emit('stt_result', { transcript: r.alternatives[0].transcript, isFinal: r.isFinal });
        });
    });
    socket.on('audio_chunk', (chunk) => {
        if (recognizeStream && !recognizeStream.destroyed)
            ...
            socket.emit('stt_result', { transcript: `(STT 연결 대기 중...) ${chunk.length}bytes`, isFinal: false });
    });
    ...
    socket.on('request_next_question', async () => {
        if (sessionInterviewType === 'individual' && hasOpenAIKey && !followupAskedForCurrent && lastMainQA) {
            ...
            try {
                ...
                if (fq) {
                    socket.emit('next_question', { question: fq, isEnd: false, isFollowup: true });
                    return;
                }
            }
            catch (err) {
                console.error('⚠️ [Followup]', err);
            }
        }
        followupAskedForCurrent = false;
        lastMainQA = null;
    ...
    socket.on('submit_answer', async (data) => {
        ...
        if (!isFollowup)
            lastMainQA = { question: data.question, answer: data.answer };
        ...
        if (hasOpenAIKey && answerLength > 0) {
            try {
                const fb = await generateFeedback(data.question, data.answer, focusScore, sessionLanguage, isFollowup);
                score = fb.score;
                good = fb.good;
                bad = fb.bad;
            }
            catch (err) {
                console.error('⚠️ [Feedback]', err);
            }
        }
        else {
            if (focusScore >= 80)
    ...
    socket.on('pt_submit_presentation', async (data) => {
        ...
        if (hasOpenAIKey && data.transcript.trim().length > 0) {
            try {
                ...
                good = r.good || good;
                bad = r.bad || bad;
                if (Array.isArray(r.qa) && r.qa.length >= 2)
                    qaQs = r.qa.slice(0, 2);
            }
            catch (err) {
                console.error('⚠️ [PT Feedback]', err);
            }
        }
        ptQAQuestions = qaQs;
        ptQAIndex = 0;
    ...
    socket.on('pt_submit_qa_answer', async (data) => {
        const { totalFrames, lookAwayFrames } = data.analysis || {};
        const focusScore = totalFrames > 0 ? Math.round(((totalFrames - lookAwayFrames) / totalFrames) * 100) : 0;
        ...
        if (hasOpenAIKey && data.answer.trim().length > 0) {
            try {
                ...
                console.error('⚠️ [PT QA Feedback]', err);
            }
        }
    ...
    socket.on('create_debate_room', (data) => {
    ...
    socket.on('join_debate_room', (data) => {
        ...
        if (!room) {
            socket.emit('debate_room_error', { error: '방 코드를 다시 확인해주세요.' });
            return;
        }
        if (room.status !== 'waiting') {
            socket.emit('debate_room_error', { error: '이미 시작된 토론입니다.' });
            return;
        }
        if (room.participants.length >= 8) {
            socket.emit('debate_room_error', { error: '방이 가득 찼습니다.' });
            return;
        }
        if (room.isPrivate && room.password && room.password !== data.password) {
            socket.emit('debate_room_error', { error: '비밀번호가 틀렸습니다.' });
            return;
        }
        const name = data.name.trim();
        if (room.participants.some(p => p.name === name)) {
            socket.emit('debate_room_error', { error: '이미 사용 중인 이름입니다.' });
            return;
        }
    ...
    socket.on('start_debate', async (data) => {
        ...
        const buildRoundOrder = () => {
            if (room.debateType === 'pro-con') {
                ...
                for (let i = 0; i < max; i++) {
                    if (pros[i])
                        order.push(pros[i].socketId);
                    if (cons[i])
                        order.push(cons[i].socketId);
                }
                return order;
            }
    ...
    socket.on('debate_submit_speech', async (data) => {
        const room = debateRooms.get(data.code);
        ...
        if (room.currentSpeakerIndex < room.roundOrder.length) {
            ...
            if (room.currentRound < room.totalRounds) {
                ...
                room.currentSpeakerIndex = 0;
                // 라운드마다 찬반 순서 뒤집기
                if (room.debateType === 'pro-con' && room.currentRound % 2 === 0)
                    room.roundOrder = [...room.roundOrder].reverse();
                else
                    room.roundOrder = [...room.roundOrder].sort(() => Math.random() - 0.5);
                ...
                const results = { topic: room.topic, participants: room.participants, speeches: room.speeches, evaluations: [] };
                if (hasOpenAIKey) {
                    try {
                        ...
                        if (Array.isArray(r.evaluations))
                            results.evaluations = r.evaluations;
                    }
                    catch { }
                }
                // 점수 기반 평균 fallback
                room.participants.forEach(p => {
                    if (!results.evaluations.find((e) => e.participantId === p.socketId)) {
                        const pSpeeches = room.speeches.filter(s => s.participantId === p.socketId);
    ...
    socket.on('join_group_room', (data) => {
        ...
        if (!room) {
            socket.emit('group_room_error', { error: '방 코드를 다시 확인해주세요.' });
            return;
        }
        if (room.status !== 'waiting') {
            socket.emit('group_room_error', { error: '이미 시작된 면접입니다.' });
            return;
        }
        if (room.participants.length >= 8) {
            socket.emit('group_room_error', { error: '방이 가득 찼습니다. (최대 8명)' });
            return;
        }
        if (room.isPrivate && room.password && room.password !== data.password) {
            socket.emit('group_room_error', { error: '비밀번호가 틀렸습니다.' });
            return;
        }
        const trimmedName = data.name.trim();
        if (room.participants.some(p => p.name === trimmedName)) {
            socket.emit('group_room_error', { error: '이미 사용 중인 이름입니다.' });
            return;
        }
    ...
    socket.on('group_submit_answer', async (data) => {
        const room = groupRooms.get(data.code);
        if (!room || room.status !== 'interviewing')
            return;
        const answerer = room.participants[room.currentAnswererIndex];
        if (answerer?.socketId !== socket.id)
            return;
        const { totalFrames, lookAwayFrames } = data.analysis;
        const focusScore = totalFrames > 0 ? Math.round(((totalFrames - lookAwayFrames) / totalFrames) * 100) : 0;
        const answerLength = data.answer?.trim().length || 0;
        ...
        if (hasOpenAIKey && answerLength > 0) {
            try {
                const fb = await generateFeedback(room.questions[room.currentQuestionIndex], data.answer, focusScore, 'ko-KR', false);
                score = fb.score;
                good = fb.good;
                bad = fb.bad;
            }
        ...
        if (!room.results[qi])
            room.results[qi] = { question: room.questions[qi], answers: [] };
    ...
    socket.on('disconnect', () => {
        ...
        for (const [code, room] of debateRooms) {
            ...
            if (room.status === 'debating') {
                ...
                if (room.currentSpeakerIndex >= room.roundOrder.length)
                    room.currentSpeakerIndex = 0;
                if (room.roundOrder.length > 0)
                    io.to(`debate:${code}`).emit('debate_turn', { currentSpeakerId: room.roundOrder[room.currentSpeakerIndex], round: room.currentRound, totalRounds: room.totalRounds });
            }
            io.to(`debate:${code}`).emit('debate_room_updated', { room });
            broadcastDebateRoomList();
            break;
